import { d as dataMediaQueries, s as slideToggle, a as slideUp, b as bodyLockToggle$1, c as bodyLockStatus, e as bodyUnlock, g as gotoBlock, f as getHash } from "./common.min.js";
(function polyfill() {
...
const overlayController = (() => {
  ...
  function close() {
    if (!overlay) return;
    const wasMenuOpen = document.documentElement.hasAttribute("data-fls-menu-open");
    overlay.classList.remove(
      "overlay--active",
      "overlay--menu",
      "overlay--popup",
...
function initSliders() {
  const slider = document.querySelector("[data-fls-slider]");
  if (slider) {
      ...
      */
      ...
      on: {
        init(swiper2) {
          ...
          requestAnimationFrame(() => {
            const realSlides = getRealSlides(swiper2);
            const middleIndex = Math.floor(realSlides.length / 2);
            swiper2.slideToLoop(middleIndex, 0, false);
            updateActiveSlide(swiper2);
    ...
    initSwiperClickNavigation({
      swiper,
      containerSelector: ".testimonials__wrapper",
      itemSelector: ".testimonials__slide"
    });
  }
}
function getRealSlides(swiper) {
  return [...swiper.slides].filter(
    (s) => !s.classList.contains("swiper-slide-duplicate")
  );
}
function updateActiveSlide(swiper) {
  const realSlides = getRealSlides(swiper);
  if (!realSlides.length) return;
  realSlides.forEach((slide2) => {
    slide2.classList.remove("testimonials__slide--active");
  });
  const realIndex = swiper.realIndex;
  const activeRealSlide = realSlides[realIndex];
  if (activeRealSlide) {
    activeRealSlide.classList.add("testimonials__slide--active");
  }
}
function initSwiperClickNavigation({
  swiper,
  containerSelector,
  itemSelector
}) {
  const container = swiper.el.querySelector(containerSelector);
  if (!container) return;
  container.addEventListener("click", (e) => {
    const item = e.target.closest(itemSelector);
    if (!item) return;
    const realSlides = getRealSlides(swiper);
    const index = realSlides.indexOf(item);
    if (index !== -1) {
...
function menuInit() {
  ...
  document.addEventListener("click", (e) => {
    const scrollBtn = e.target.closest("[data-fls-scrollto]");
    if (scrollBtn) {
      closeMenu();
    }
  });
...
function createWatcher(selector, callback, options = {}) {
...
createWatcher(
  ".logo-footer__letters",
  (entry, el) => {
    if (entry.isIntersecting) {
      const items = [
        ...el.querySelectorAll(".logo-footer__letter"),
        el.querySelector(".logo-footer__wrapper-i")
      ].filter(Boolean);
      ...
      const items = el.querySelectorAll(
        ".logo-footer__letter, .logo-footer__wrapper-i"
      );
      items.forEach((item) => {
...
marquee();
function initDragScroll(selector, speed = 1, momentum = 0.97, maxVelocity = 40) {
  ...
  let velocity = 0;
  let rafId;
  const getX = (e) => e.pageX - slider.offsetLeft;
...
function initActiveCard({
  ...
  const getEl = (item) => innerSelector ? item.querySelector(innerSelector) : item;
...
initActiveCard({
  itemSelector: ".works__item",
  innerSelector: ".works__button",
  activeClass: "works__button--active",
  onActivate: (el) => el.textContent = "View",
  onDeactivate: (el) => el.textContent = "View Case"
});
initDragScroll(".works__list", 1, 0.97, 40);
initActiveCard({
  itemSelector: ".services__item",
  innerSelector: ".icon--arrow-big",
  activeClass: "icon--arrow-active"
});
initActiveCard({
  itemSelector: "[data-fls-itemprising]",
  activeClass: "item-pricing--active"
});
createWatcher(
  ".works",
  (entry, el) => {
...
createWatcher(
  ...
  (entry, el) => {
    if (entry.isIntersecting) {
      const items = el.querySelectorAll(".item-pricing");
      items.forEach((item, index) => {
        item.style.transitionDelay = `${0.15 + index * 0.12}s`;
      });
      el.classList.add("animate");
    }
  },
